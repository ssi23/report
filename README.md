...
**클라이언트가 요청하면 서버에서 응답해주는 방식의 서버측 프로그램**으로 마이크로소프트가 제작한 서버 사이드 스크립트 엔진이다. <br>
이를 사용하면 서버에서 웹을 프로그래밍 할 수 있으며, 기존의 HTML 페이지와는 다른 동적인 구성을 할 수 있다.<br>
브라우저가 웹 서버에서 ASP 파일을 요청하면 서버는 프로세서를 호출하고, 프로세서는 요청된 파일을 읽고 스크립트 명령을 실행하여 결과를 웹페이지 형태로 브라우저에 전송한다.<br><br>
...
#### ASP의 특징
* .asp 확장자를 사용한다.
* 텍스트 파일을 업데이트하는 것만으로도 웹페이지나 데이터베이스의 목차를 편집할 수 있다.   ?
* 웹 페이지 상에서 데이터베이스를 보여주고 그 안에 실제로 존재하는 데이터를 조작할 수 있다.    ?
